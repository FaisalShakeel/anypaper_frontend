...
const Chat = () => {
  ...
  const fetchChatMessages = async (chatId) => {
    console.log('fetchChatMessages called for chatId:', chatId); // Debug log
    ...
    try {
      const response = await axios.get(
        ...
        { params: { userId: user.id, role: 'admin' } }
      );
  ...
  const handleChatSelect = async (chat) => {
    ...
    // Always fetch messages from backend to ensure latest data
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        toast.error('Unsupported file type. Please upload an image, video, PDF, or Word document.');
  ...
  const handleSendFile = async () => {
    console.log('handleSendFile called', { text, selectedFile }); // Debug log
    ...
    if (!user?.id || !selectedChat?.otherUser?._id || !selectedChat?._id) {
      const errorMsg = 'Cannot send message: Invalid user or chat data';
    ...
    try {
      ...
      if (!newMessage) {
        const errorMsg = 'No message returned from server';
        setError(errorMsg);
  ...
  const getAllChats = async () => {
    ...
    try {
      ...
      const errorMsg = e.response?.data?.message || 'Failed to load chats';
      setError(errorMsg);
      toast.error(errorMsg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    if (isSmallDevice) {
      setMobileView(selectedChat ? 'messages' : 'chats');
    } else {
      setMobileView('chats');
    }
  ...
  if (error) {
    return (
      <Box sx={{ p: 3, bgcolor: '#f9f9f9' }}>
        <ErrorMessage message={error} onTryAgain={getAllChats} />
      </Box>
    );
  }
    ...
    >
      ...
      <Box
        sx={{
          ...
          borderRadius: 2,
          padding: 2,
          display: isSmallDevice && mobileView === 'messages' ? 'none' : 'block',
        }}
      >
        ...
        >
          {filteredChats.length === 0 ? (
            ...
            filteredChats
              .filter((chat) => {
                if (!chat?.roles) return false;
                if (activeTab === 'Students And Writers') {
                  return (
                    (chat.roles[0] === 'student' || chat.roles[0] === 'writer') &&
                    (chat.roles[1] === 'student' || chat.roles[1] === 'writer')
                  );
                } else if (activeTab === 'Students And Editors') {
                  return (
                    (chat.roles[0] === 'student' || chat.roles[0] === 'editor') &&
                    (chat.roles[1] === 'student' || chat.roles[1] === 'editor')
                  );
                } else if (activeTab === 'Editors And Writers') {
                  return (
                    (chat.roles[0] === 'editor' || chat.roles[0] === 'writer') &&
                    (chat.roles[1] === 'editor' || chat.roles[1] === 'writer')
                  );
                }
                return true;
              })
                ...
                >
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                    <ListItemAvatar>
                      ...
                      >
                        {chat.lastMessage?.sender?.role === 'writer'
                          ? 'W'
                          : chat.lastMessage?.sender?.role === 'editor'
                          ? 'E'
                          : 'S'}
                      </Avatar>
                    </ListItemAvatar>
      ...
      >
        {selectedChat ? (
                  ...
                  {selectedChatMessages.length === 0 ? (
                    ...
                    selectedChatMessages.map((msg) => (
                      ...
                      >
                          ...
                          sx={{
                            bgcolor:
                              msg.sender?.userId === user?.id
                                ...
                                : '#F7F8FA',
                            color:
                              msg.sender?.userId === user?.id ? 'black' : 'black',
                            padding: 2,
                        ...
                        >
                          {convertIntoTimeFormat(msg.createdAt || msg.timestamp)}
                        </Typography>
                      </Box>
                    ))
                  )}
          ...
          <Typography sx={{ textAlign: 'center', marginTop: 'auto', marginBottom: 'auto' }}>
