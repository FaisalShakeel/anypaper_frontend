...
const MyFiles = () => {
  ...
  const getMyFiles = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/editors/my-files`, {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_BACKEND_BASE_URL}/editors/add-to-my-files`, formData, {
        withCredentials: true,
  ...
  const handleDelete = async (fileId, fileName) => {
    ...
    console.log(`Attempting to delete file: ID=${fileId}, Name=${fileName}, Type=${card.fileType}`);

    // Prevent multiple deletions if already deleting
    if (deletingFiles.has(fileId)) {
      console.log(`File ID=${fileId} is already being deleted.`);
      return;
    }

    setDeletingFiles((prev) => {
      const newSet = new Set([...prev, fileId]);
      console.log(`Added ID=${fileId} to deletingFiles:`, [...newSet]);
      return newSet;
    });

    try {
      const response = await axios.delete(`${process.env.REACT_APP_BACKEND_BASE_URL}/editors/delete-file/${fileId}/${card.fileType}`, {
        withCredentials: true
      });

      console.log(`Delete response for ID=${fileId}:`, response.data);
      ...
      if (response.data.success) {
        ...
        setFiles((prevFiles) => {
          const newFiles = { ...prevFiles };
          if (card.fileType === 'MyUploads') {
            newFiles.myUploads = prevFiles.myUploads.filter(file => file.id !== fileId);
          } else if (card.fileType === 'SolutionFiles') {
            newFiles.solutionFiles = prevFiles.solutionFiles.filter(file => file.id !== fileId);
          } else if (card.fileType === 'DraftFiles') {
            newFiles.draftFiles = prevFiles.draftFiles.filter(file => file.id !== fileId);
          }
          console.log(`Updated files after deletion of ID=${fileId}:`, newFiles);
          return newFiles;
        });
      } else {
      ...
      setDeletingFiles((prev) => {
        ...
        console.log(`Removed ID=${fileId} from deletingFiles`);
        return newSet;
      });
    }
  };
                  ...
                  <TableCell>
                    ...
                    >
                      {deletingFiles.has(file.id) ? (
                        <>
                          {console.log(`Rendering spinner for file ID=${file.id}`)}
