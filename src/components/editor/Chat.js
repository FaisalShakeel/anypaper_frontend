...
const Chat = () => {
  ...
  const [caption, setCaption] = useState(''); // New state for preview caption
  ...
  const fetchChatMessages = async (chatId) => {
    ...
    try {
      const response = await axios.get(
        ...
        { params: { userId: user.id, role: 'editor' } }
      );
  ...
  const handleChatSelect = async (chat) => {
    if (!chat || !chat._id || !chat.otherUser) return;
  ...
  const handleSendFile = async () => {
    ...
    try {
      ...
      formData.append('senderRole', 'editor');
  ...
  const getAllChats = async () => {
    ...
    try {
      const response = await axios.get(
        `${
          process.env.REACT_APP_BACKEND_BASE_URL || 'http://localhost:5000'
        }/chats/all-chats/${user.id}/editor`,
        { params: { userId: user.id, role: 'editor' } }
      );

      const chats = response.data?.chats;
      if (!chats) {
        const errorMsg = 'No chats returned from server';
        setError(errorMsg);
        toast.error(errorMsg);
        setAllChats([]);
      } else {
                  ...
                  filteredChats
                    .filter((chat) => {
                      ...
                      if (activeTab === 'Students') {
                        return chat.otherUserRole === 'student';
                      ...
                      >
                        ...
                        >
                          <ListItemAvatar>
                            ...
                            >
                              {chat?.otherUserRole === 'student' ? 'S' : 'W'}
                            </Avatar>
                          </ListItemAvatar>
                          <Box sx={{ flex: 1, ml: 1 }}>
                            <Typography
