...
const MyFiles = () => {
  ...
  const getMyFiles = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/writers/my-files`, {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_BACKEND_BASE_URL}/writers/add-to-my-files`, formData, {
        withCredentials: true,
  ...
  const handleDelete = async (fileId, fileName) => {
    ...
    try {
      const response = await axios.delete(`${process.env.REACT_APP_BACKEND_BASE_URL}/writers/delete-file/${fileId}/${card.fileType}`, {
        withCredentials: true
      });
