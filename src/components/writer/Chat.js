...
const Chat = () => {
  ...
  const fetchChatMessages = async (chatId) => {
    ...
    try {
      const response = await axios.get(
        ...
        { params: { userId: user.id, role: 'writer' } }
      );
  ...
  const handleSendFile = async () => {
    ...
    try {
      ...
      formData.append('senderRole', 'writer');
  ...
  const getAllChats = async () => {
    ...
    try {
      const response = await axios.get(
        `${
          process.env.REACT_APP_BACKEND_BASE_URL || 'http://localhost:5000'
        }/chats/all-chats/${user.id}/writer`,
        { params: { userId: user.id, role: 'writer' } }
      );
            ...
            {loading ? (
              ...
              filteredChats
                .filter((chat) => {
                  ...
                  } else if (activeTab === 'Support Team') {
                  ...
                  >
                    ...
                    >
                      <ListItemAvatar>
                        ...
                        >
                          {chat?.otherUserRole === 'student' ? 'S' : 'E'}
                        </Avatar>
                      </ListItemAvatar>
                      <Box sx={{ flex: 1, ml: 1 }}>
                        <Typography
